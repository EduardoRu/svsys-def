        ...
        var dd = {
          ...
          content: [
            ...
            },
            {
              columns: [
                {
                  text: 'OBSERVACIONES DEL INSPECTOR:' + estudioMtro.observaciones, fontSize: 8
                },
                {
                  text: 'CUMPLE CON LA NORMA NOM-010-SCFI-1994 CUMPLE (C) NO CUMPLE (NC)', fontSize: 8, width: 220
  ...
  async generarCliente(
    ...
    if (
      ...
      this.obtenerUbicacion().then((res: any) => {
        ...
        pdfDocGenerator.getBlob(async (blob) => {
          const fileName = `${infoCliente.nombre_razon_social}_${new Date().getTime()}.pdf`;
          const url = await this.uploadPDF(blob, fileName);
          await this.presentQrModal(url);
          console.log('Enlace del PDF:', url);
        });
